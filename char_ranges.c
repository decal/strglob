...
/*! @fn size_t char_ranges(CHAR_RANGE *crang, STR_GLOB *ugcrn)
 *
 ...
 *
 *  @return the total number of characters represented by the supplied character range data structures
 */

size_t char_ranges(const CHAR_RANGE *const crang, STR_GLOB *ugcrn) {
  assert(crngs);
  assert(ugcrn);
